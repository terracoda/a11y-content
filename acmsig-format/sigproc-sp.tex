...
\maketitle
\begin{abstract}
\textit{Ramp it up} is an action-based guide to creating accessible websites. It is a visualization aid designed to help web teams integrate accessible design choices into their practices, thus resulting in websites that are more likely to comply with the W3C's Web Content Accessibility Guidelines (WCAG 2.0). The poster aims to help all members of the web development team - designers, developers, writers and project managers. Each stair represents a content type or design decision that could potentially introduce a barrier to a person with a disability. Associated with each stair is a list of accessible techniques that, when implemented, builds in the virtual the ramp ensuring access to that website content. The action-based techniques (or ramp) focuses on making sites that are perceivable, operable, understandable and robust. By focusing on the guidelines' POUR principles we hope teams are better equipped to integrate accessible practices without getting lost in highly technical and sometimes confusing language of the success criteria. 
